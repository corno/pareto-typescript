import * as p_ from 'pareto-core/dist/implementation/refiner'
import * as p_pi from 'pareto-core/dist/interface/production'
import * as p_di from 'pareto-core/dist/interface/data'
import p_iterate from 'pareto-core/dist/implementation/refiner/specials/iterate'
import P_unreachable_code_path from 'pareto-core/dist/implementation/transformer/specials/unreachable_code_path'

//data types
import * as d_in from "./modules/typescript_parser/interface/data/ast"
import * as d_function from "./interface/data/typed_ast_from_ast"
import { Abort } from 'pareto-core/dist/interface/__internal/Abort'

export namespace iterator_helpers {

    export const consume_and_expect_deprecated = <T extends p_di.Value>(
        iterator: p_pi.Iterator<d_in.Node, null>,
        abort: Abort<d_function.Error_Inner>,
        parent: d_in.Node,
        location_description: string,
        kind: string,
        callback: (node: d_in.Node, parent: d_in.Node) => T
    ): T => iterator.consume(
        ($) => abort({
            'context': location_description,
            'parent': parent,
            'cause': ['end of node list', null],
            'expected': ['something', kind]
        }),
        ($) => node_helpers.expect_specific_node(
            $,
            abort,
            parent,
            location_description,
            kind,
            callback
        )
    )

    export type Parse_Context = {
        temp_parent: d_in.Node
        consume: <T extends p_di.Value>(
            location_description: string,
            callback: (
                node: d_in.Node,
                parent: d_in.Node,
            ) => T
        ) => T
        consume_and_expect: <T extends p_di.Value>(
            location_description: string,
            kind: string,
            callback: (
                node: d_in.Node,
                parent: d_in.Node,
            ) => T
        ) => T
        consume_list_with_syntaxlist_wrapper: <T extends p_di.Value>(
            location_description: string,
            callback: (
                node: d_in.Node,
                parent: d_in.Node,
                context: {
                    abort: Abort<string>
                }
            ) => T
        ) => p_di.List<T>
        call: <T extends p_di.Value>(
            func: p_pi.Production_With_Parameter<
                T,
                d_function.Error_Inner,
                d_in.Node,
                null,
                {
                    'parent': d_in.Node
                }
            >
        ) => T
        optional: <T extends p_di.Value>(
            condition: ($: d_in.Node) => boolean,
            callback: (context: Parse_Context) => T
        ) => p_di.Optional_Value<T>
    }

    export const create_parse_context = <T extends p_di.Value>(
        iterator: p_pi.Iterator<d_in.Node, null>,
        abort: Abort<d_function.Error_Inner>,
        parent: d_in.Node,
        callback: (context: Parse_Context) => T
    ): T => {
        const consume_deprecated = <T extends p_di.Value>(
            iterator: p_pi.Iterator<d_in.Node, null>,
            abort: Abort<d_function.Error_Inner>,
            parent: d_in.Node,
            location_description: string,
            callback: (node: d_in.Node, parent: d_in.Node) => T
        ): T => iterator.consume(
            ($) => abort({
                'context': location_description,
                'parent': parent,
                'cause': ['end of node list', null],
                'expected': ['something', location_description]
            }),
            ($) => callback(
                $,
                parent,
            )
        )
        return callback({
            temp_parent: parent,
            consume: (
                location_description,
                callback
            ) => {
                return consume_deprecated(
                    iterator,
                    abort,
                    parent,
                    location_description,
                    callback
                )
            },
            consume_and_expect: (
                location_description,
                kind,
                callback
            ) => {
                return iterator_helpers.consume_and_expect_deprecated(
                    iterator,
                    abort,
                    parent,
                    location_description,
                    kind,
                    callback
                )
            },
            consume_list_with_syntaxlist_wrapper: (
                location_description,
                callback
            ) => {

                return consume_deprecated(
                    iterator,
                    abort,
                    parent,
                    location_description,
                    ($) => {
                        const my_node = $
                        return node_helpers.list_with_syntaxlist_wrapper(
                        $,
                        parent,
                        abort,
                        location_description,
                        ($) => callback(
                            $,
                            parent,
                            {
                                'abort': ($) => abort({
                                    'parent': parent,   
                                    'cause': ['unexpected node', my_node],
                                    'expected': ['something', $],
                                    'context': location_description
                                })
                            }
                        )

                    )
                    }
                )
            },
            call: (
                func
            ) => {
                return func(
                    iterator,
                    abort,
                    {
                        'parent': parent
                    }
                )
            },
            optional: (
                condition,
                callback
            ) => {
                return iterator.peek(
                    () => p_.literal.not_set(),
                    ($) => {
                        if (condition($)) {
                            return p_.literal.set(create_parse_context(
                                iterator,
                                abort,
                                parent,
                                callback
                            ))
                        } else {
                            return p_.literal.not_set()
                        }
                    }
                )
            }
        })
    }

}


export namespace node_helpers {

    export const expect_specific_node = <T extends p_di.Value>(
        $: d_in.Node,
        abort: Abort<d_function.Error_Inner>,
        parent: d_in.Node,
        location_description: string,
        kind: string,
        callback: (node: d_in.Node, parent: d_in.Node) => T

    ): T => $.kind !== kind
            ? abort({
                'parent': parent,
                'context': location_description,
                'cause': ['unexpected node', $],
                'expected': ['something', kind]
            })
            : callback($, parent)

    export const list_with_syntaxlist_wrapper = <T extends p_di.Value>(
        $: d_in.Node,
        parent: d_in.Node,
        abort: Abort<d_function.Error_Inner>,
        location_description: string,
        callback: (node: d_in.Node) => T
    ): p_di.List<T> => $.kind !== "SyntaxList"
            ? abort({
                'parent': parent,
                'context': location_description,
                'cause': ['unexpected node', $],
                'expected': ['something', `SyntaxList (${location_description})`]
            })
            : p_iterate({
                list: $.children,
                end_info: null,
                assign: (iterator) => iterator.build_list({
                    'has_more_items': ($) => true,
                    'handle': () => iterator.consume(
                        ($) => P_unreachable_code_path("has more items -> true"),
                        ($): T => callback($),
                    )
                }),
                on_dangling_item: ($) => abort({
                    'context': location_description,
                    'parent': parent,
                    'cause': ['unexpected node', $],
                    'expected': ['nothing', null]
                }),
            })



    export const process_children_as_list = <T extends p_di.Value>(
        $: d_in.Node,
        callback: (node: d_in.Node, parent: d_in.Node) => T
    ): p_di.List<T> => {
        const my_node = $
        return p_iterate({
            list: $.children,
            end_info: null,
            assign: (iterator) => iterator.build_list({
                'has_more_items': ($) => true,
                'handle': () => iterator.consume(
                    ($) => P_unreachable_code_path("has more items -> true"),
                    ($): T => callback($, my_node),
                )
            }),
            on_dangling_item: ($) => P_unreachable_code_path("build_list processes all items"),
        })
    }

    export const process_children_as_group = <T extends p_di.Value>(
        $: d_in.Node,
        abort: Abort<d_function.Error_Inner>,
        location_description: string,
        callback: (
            context: iterator_helpers.Parse_Context
        ) => T
    ): T => {
        const parent = $

        return p_iterate({
            list: $.children,
            end_info: null,
            assign: (iterator): T => iterator_helpers.create_parse_context(
                iterator,
                abort,
                parent,
                (context) => callback(
                    context
                )
            ),
            on_dangling_item: ($) => abort({
                'parent': parent,
                'context': location_description,
                'cause': ['unexpected node', $],
                'expected': ['nothing', null]
            }),
        })
    }

}
